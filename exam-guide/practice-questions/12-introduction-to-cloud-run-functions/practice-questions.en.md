...
B. No — Cloud Run functions (2nd gen) deploys functions as Cloud Run services under the hood, which is also why a function can be moved to plain Cloud Run or Kubernetes if needed.
C. Yes, but only for HTTP functions; event-driven functions run on a different platform entirely.
...
B. An HTTP function; it is assigned a URL to receive requests, and by default requests to it require authentication (unauthenticated access can be enabled at deployment).
C. A Background function; URLs are only assigned to Background functions, never to HTTP functions.
D. An HTTP function; authentication is never available for HTTP functions, only for event-driven functions.
...
A. Background functions, since they are the newer of the two styles.
B. CloudEvent functions, based on the CloudEvents industry-standard specification and registered with the Functions Framework, an open-source library that wraps user functions in a persistent HTTP application.
...
B. Only on Cloud Run functions 1st gen, and only with the Node.js, Python, Go, and Java runtimes.
C. Only on Cloud Run functions (2nd gen), and only with .NET, Ruby, and PHP.
...
A. The source code is not defined in a file named `index.js` at the root of the function directory.
B. Node.js functions must always be named `main.py` regardless of language.
C. Node.js does not support Cloud Run functions at all.
D. The `package.json` file was deleted, which is unrelated to where Cloud Run functions looks for source code.
...
A. No — no Cloud Run function can run longer than 10 minutes under any circumstances.
B. Yes — HTTP functions support a run time limit of up to 60 minutes, versus up to 10 minutes for event-driven functions.
...
A. Each instance can process up to 1000 concurrent requests, which reduces cold starts and improves overall latency when scaling.
B. Cloud Run functions instances can only ever process one request at a time, by design.
C. Concurrency is configurable up to 10 requests per instance, with no effect on cold starts.
D. Concurrency only applies to event-driven functions, never to HTTP functions.
...
B. The user also needs the Service Account User IAM role on the Cloud Run functions runtime service account.
C. The user needs Organization Admin, since function deployment always requires organization-level permissions.
...
A. The zip's source files must be at the root of the zip file, and the deploying account (1st gen) or the Cloud Run functions service agent (2nd gen) needs permission to read from the bucket.
B. Cloud Storage cannot be used as a source location; only local machine and source repositories are supported.
...
A. There is no way to deploy from a subdirectory; the entire repository root must always be used.
B. The source repository path includes `revisions/<revision_name>` for the revision, with `paths/<source_directory_path>` appended to point at the subdirectory; the Cloud Run functions service agent needs the Source Repository Reader (`roles/source.reader`) role on the repository.
...
B. Latency and availability are the primary considerations, but the location of the other Google Cloud services the app depends on also matters, since services spread across multiple locations can affect both latency and pricing.
C. Region selection has no effect on pricing under any circumstances.
D. Cloud Run functions can only be deployed to a single global region, so region selection is not a real decision.
...
B. The source is stored in a Cloud Storage bucket, then Cloud Build automatically builds it into a container image and pushes that image to Artifact Registry, which Cloud Run functions then accesses to run the function.
C. Cloud Scheduler builds the container image, and Cloud Tasks stores it.
