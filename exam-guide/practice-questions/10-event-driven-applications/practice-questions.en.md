...
A. Events are typically treated as immutable facts — a historical record of an occurrence that should not be modified or deleted. The correction should be a new event, not an edit to the old one.
B. This is fine — events can always be edited in place as long as the change is documented.
...
B. Yes, but only because events are expensive to store, so unconsumed events waste resources.
C. No — an event can be generated even if it's never consumed; many producers don't know, or need to know, whether anything is consuming their events.
...
B. Yes — an event can be persisted indefinitely and consumed as many times as necessary, including in parallel by multiple services.
C. No — events expire and are purged shortly after being produced, so reprocessing old events isn't possible.
...
B. This is solved by adding more microservices, which automatically reduces coupling.
C. This is solved by merging Services B, C, and D back into a single monolith.
D. This is the point-to-point "spider web" coupling problem; inserting an event intermediary between services lets producers stop needing to know which services consume their events.
...
A. It isn't necessary for the producer to know anything about the services consuming its events — it only needs to know the event's format.
B. It needs to know the network address and API contract of every consumer in advance.
C. It needs to know how many consumers exist so it can fan out requests correctly.
...
B. No — an event intermediary requires messages to be synchronous, while an ESB only supports asynchronous messaging.
C. No — an ESB is a centralized routing/transformation layer that became a bottleneck because every integration change had to go through it and its owning team; an event intermediary specifically exists to decouple producers from consumers without recreating that kind of centralized change bottleneck.
...
B. Only the event producer can enforce access control; consumers have no way to restrict who reads events.
C. Access control is unnecessary in event-driven systems because events are immutable.
D. A centralized event service can require authentication and authorization at the point where events flow through it, letting you control access to event-based services and data from one place.
...
A. Yes — because producers and consumers are decoupled and only need to agree on an event's format, a new event consumer can be added without modifying any existing services.
B. No — any new consumer requires updating the producer to add the new service to its list of recipients.
...
A. This isn't possible — any events produced while a consumer was down are permanently lost.
B. Events sent to an unhealthy service can be replayed or redelivered once that service comes back up, because events are persisted rather than discarded after a single delivery attempt.
...
B. This is called replay, and its downside is that it can only be used for consumer recovery after downtime, not for normal operation.
C. This is the event intermediary pattern, and there is no real downside compared to alternatives.
D. This is polling; it typically increases network I/O and adds unnecessary delay before new work is processed. Push-based messaging, where consumers are automatically notified when there's something to consume, is preferred.

**13.** With push-based messaging, how does a consumer typically find out that a new event is available to process?

A. The consumer is automatically notified by the intermediary when there's an event to consume, rather than having to ask.
B. The consumer repeatedly queries the event intermediary on a fixed schedule until it finds new data.
...
B. No — producers and consumers must share the same programming language and deployment platform.
C. No — a producer or consumer is only required to know the format of a specific event; that shared understanding of event format is still necessary even though neither side needs to know about the other's identity or implementation.
