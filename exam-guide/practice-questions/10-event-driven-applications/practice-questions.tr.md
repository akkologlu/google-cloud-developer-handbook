...
A. Bu sorun değil — değişiklik dokümante edildiği sürece event'ler her zaman yerinde düzenlenebilir.
B. Event'ler genelde immutable (değişmez) gerçekler olarak ele alınır — değiştirilmemesi ya da silinmemesi gereken bir olayın tarihsel kaydıdır. Düzeltme, eskisini düzenlemek yerine yeni bir event olarak ifade edilmelidir.
...
B. Hayır — bir event, hiç consume edilmese bile üretilebilir; birçok producer, event'lerini bir şeyin consume edip etmediğini bilmez, bilmesine de gerek yoktur.
C. Evet, ama yalnızca event'leri saklamak pahalı olduğu için, consume edilmeyen event'ler kaynak israf eder.
...
B. Hayır — event'ler üretildikten kısa süre sonra süresi dolar ve temizlenir, bu yüzden eski event'leri yeniden işlemek mümkün değildir.
C. Evet — bir event süresiz olarak persist edilebilir ve gerektiği kadar çok kez, birden fazla servis tarafından paralel olarak da dahil, consume edilebilir.
...
B. Bu, point-to-point iletişimin "örümcek ağı" coupling sorunudur; servisler arasına bir event intermediary eklemek, producer'ların event'lerini kimin consume ettiğini bilmesine gerek kalmamasını sağlar.
C. Bu, otomatik olarak coupling'i azaltan daha fazla microservice eklenerek çözülür.
D. Bu, Service B, C ve D'nin tekrar tek bir monolith'e birleştirilmesiyle çözülür.
...
A. Önceden her consumer'ın network adresini ve API contract'ını bilmesi gerekir.
B. İstekleri doğru şekilde dağıtabilmek için kaç consumer olduğunu bilmesi gerekir.
C. Producer'ın event'lerini consume eden servisler hakkında hiçbir şey bilmesine gerek yoktur — yalnızca event'in formatını bilmesi gerekir.
...
B. Hayır — ESB, her entegrasyon değişikliğinin kendisinden ve onu yöneten takımdan geçmesi gerektiği için bir darboğaza dönüşen merkezi bir routing/transformation katmanıydı; event intermediary ise özellikle producer'ları consumer'lardan, bu tür merkezi bir değişiklik darboğazını yeniden yaratmadan ayırmak için var olur.
C. Hayır — bir event intermediary mesajların senkron olmasını gerektirir, ESB ise yalnızca asenkron mesajlaşmayı destekler.
...
B. Merkezi bir event service, event'lerin kendisinden geçtiği noktada authentication ve authorization'ı zorunlu kılabilir, bu da event-based servislere ve verilere erişimi tek bir yerden kontrol etmeni sağlar.
C. Yalnızca event producer access control'ü zorunlu kılabilir; consumer'ların event'leri kimin okuduğunu kısıtlamasının bir yolu yoktur.
D. Event-driven sistemlerde access control gereksizdir çünkü event'ler immutable'dır.
...
A. Hayır — herhangi bir yeni consumer, producer'ın alıcı listesine yeni servisi eklemek için güncellenmesini gerektirir.
B. Evet — çünkü producer'lar ve consumer'lar decoupled'dır ve yalnızca bir event'in formatı üzerinde anlaşmaları gerekir, mevcut hiçbir servisi değiştirmeden yeni bir event consumer eklenebilir.
...
A. Sağlıksız bir servise gönderilen event'ler, o servis geri geldiğinde replay edilebilir ya da yeniden teslim edilebilir, çünkü event'ler tek bir teslimat denemesinden sonra atılmak yerine persist edilir.
B. Bu mümkün değildir — bir consumer devredeyken üretilen tüm event'ler kalıcı olarak kaybolur.
...
B. Bu polling'dir; genelde network I/O'yu artırır ve yeni işin işlenmesinden önce gereksiz gecikme ekler. Consumer'ların consume edilecek bir şey olduğunda otomatik olarak bilgilendirildiği push-based messaging tercih edilir.
C. Buna replay denir ve dezavantajı, yalnızca kesinti sonrası consumer recovery için kullanılabilmesi, normal operasyon için değil.
D. Bu event intermediary desenidir ve alternatiflere kıyasla gerçek bir dezavantajı yoktur.

**13.** Push-based messaging'de, bir consumer genelde işlenecek yeni bir event olduğunu nasıl öğrenir?

A. Consumer, yeni veri bulana kadar event intermediary'yi sabit bir programa göre tekrar tekrar sorgular.
B. Consumer, sormak zorunda kalmak yerine, consume edilecek bir event olduğunda intermediary tarafından otomatik olarak bilgilendirilir.
...
B. Hayır — bir producer ya da consumer'ın yalnızca belirli bir event'in formatını bilmesi gerekir; hiçbir taraf diğerinin kimliğini ya da implementasyonunu bilmek zorunda olmasa bile, bu paylaşılan format anlayışı hâlâ gereklidir.
C. Hayır — producer'lar ve consumer'lar aynı programlama dilini ve deployment platformunu paylaşmalıdır.
