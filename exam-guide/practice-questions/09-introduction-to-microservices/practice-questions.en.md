...
A. Service-Oriented Architecture; the fragility comes from ESB misconfiguration.
B. A monolith; the fragility comes from tight coupling between parts of a single, ever-growing codebase.
...
B. It moved into the Enterprise Service Bus (ESB) integrations, typically owned by one central team, so nearly every application or service change required ESB work and competed for that team's time.
C. It disappeared — SOA fully eliminates integration complexity by design.
...
B. Start with a monolith, and design it modularly, since designing service boundaries is one of the hardest parts of a new microservices project and is easier once you understand the domain.
C. Start with SOA as a middle ground between monolith and microservices.
D. The decision is irrelevant — the module states architecture style has no bearing on domain expertise.
...
A. A monolith, because a single codebase is easier to onboard new engineers into as a whole.
B. Microservices, because natural service boundaries let each team member focus on a smaller, bounded piece of the system instead of the entire application.
...
B. Services interoperate through their API interface (typically HTTP), so the language, framework, or platform used inside a service is invisible to the services calling it.
C. It only works if every service is written in the same language as the ESB.
D. It works only because Kubernetes automatically translates between languages at the network layer.
...
A. Microservices always run at a fixed capacity regardless of traffic, so this scenario cannot occur.
B. Each microservice can be scaled independently, so you can dedicate more resources to checkout specifically instead of over-provisioning the entire application.
...
B. Having many more deployable entities creates a much greater operational burden; automated builds, testing, and deployment are vital to keeping this manageable.
C. Microservices should never exceed 10 services for this exact reason.
...
C. It results from poorly designed inter-service communication; the module frames this as a real risk to manage through deliberate design, not an unavoidable outcome of using microservices.
D. It's a sign the team should merge all services back into one monolith immediately.
...
A. It isn't expected — integration testing should be exactly as simple as unit testing in microservices.
B. The distributed nature of microservices generally means that testing the full system requires modeling the entire production deployment, unlike a monolith where components run in the same process.
...
B. Because each microservice creates its own logs, tracing a call that spans many microservices is inherently more challenging than tracing calls within a single monolithic process.
C. This is unrelated to architecture — it's purely a tooling budget problem.
...
B. Yes, unconditionally — microservices have no meaningful downsides once adopted.
C. Generally yes, but only with a real commitment to automation and operational excellence; without investing in that tooling, the operational burden of many independently deployable services can overwhelm the benefits.
...
**15. Correct answer: C.**
The module's own conclusion is that the benefits of microservices generally outweigh the challenges, but conditionally — only with a genuine commitment to automation and operational excellence. Presenting either extreme (A: monoliths are strictly superior, or B: microservices have no downsides) misrepresents that conditional framing.
